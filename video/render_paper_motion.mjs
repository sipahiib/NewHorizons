...
if(cliArgs.includes('--preview')){const tiles=[];for(let i=0;i<5;i++)tiles.push({input:await sharp(Buffer.from(svg(i,3.4))).resize(480,270).png().toBuffer(),left:0,top:i*270});await sharp({create:{width:480,height:1350,channels:3,background:'#08202e'}}).composite(tiles).jpeg().toFile('/private/tmp/newhorizons-paper-original-preview.jpg');}
else {for(let i=0;i<5;i++){if(cliArgs.includes('--scene-2')&&i!==1)continue;const target=path.join(OUT,names[i]+'.mp4');const proc=spawn('ffmpeg',['-nostdin','-n','-v','error','-f','rawvideo','-pix_fmt','rgb24','-s',`${W}x${H}`,'-r',`${FPS}`,'-i','pipe:0','-an','-c:v','libx264','-preset','veryfast','-crf','19','-threads','2','-pix_fmt','yuv420p','-movflags','+faststart',target],{stdio:['pipe','inherit','inherit']});let error;proc.stdin.on('error',e=>error=e);const done=once(proc,'close');for(let f=0;f<FPS*D;f++){if(error)throw error;const raw=await sharp(Buffer.from(svg(i,f/FPS))).removeAlpha().raw().toBuffer();if(!proc.stdin.write(raw))await once(proc.stdin,'drain');}proc.stdin.end();const [code]=await done;if(code)throw Error(`ffmpeg ${code}`);console.log(target);}}
